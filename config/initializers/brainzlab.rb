...
BrainzLab.configure do |config|
  ...
  config.service = "beacon"
  config.environment = Rails.env

  # Ignore internal BrainzLab hosts to prevent infinite recursion
  config.http_ignore_hosts = %w[localhost 127.0.0.1 recall reflex pulse signal flux beacon]
end
...
Rails.application.config.after_initialize do
  ...
  Rails.logger.info "[Beacon] SDK initialized"
  Rails.logger.info "[Beacon] Recall logging: #{BrainzLab.configuration.recall_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Beacon] Reflex error tracking: #{BrainzLab.configuration.reflex_enabled ? 'enabled' : 'disabled'}"
  Rails.logger.info "[Beacon] Pulse APM: #{BrainzLab.configuration.pulse_enabled ? 'enabled' : 'disabled'}"
end
