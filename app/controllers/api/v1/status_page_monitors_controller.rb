    ...
    class StatusPageMonitorsController < API::V1::BaseController
      ...
      def index
        monitors = @status_page.status_page_monitors
                              ...
                              .order(:position, :group_name)

        render json: {
          monitors: monitors.map { |spm| monitor_json(spm) },
          groups: monitors.map(&:group_name).compact.uniq
        }
      end
      ...
      def reorder
        params[:order].each_with_index do |id, index|
          @status_page.status_page_monitors.find(id).update!(position: index)
        end
      ...
      def spm_params
        params.require(:status_page_monitor).permit(
          :monitor_id, :group_name, :display_name, :position
        )
      end
      ...
      def monitor_json(spm)
        {
          ...
          monitor_name: spm.uptime_monitor.name,
          display_name: spm.display_name,
          group_name: spm.group_name,
          position: spm.position,
          current_status: spm.uptime_monitor.status
        }
      end
    end
  end
