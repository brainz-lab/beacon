...
module Ai
  class AssistantService
    ...
    def build_tools
      MCP::ToolRegistry::TOOLS.map do |tool_class|
        { name: tool_class.name, description: tool_class.description, input_schema: tool_class.parameters }
      end
    rescue => e
    ...
    def system_prompt
      "You are an AI assistant for Beacon, the uptime monitoring system. You help users manage monitors, check service status, view incident history, and configure notifications. Be concise and helpful. Format data with markdown. Confirm destructive actions first."
    end
    ...
    def execute_tool(name, arguments)
      ...
      tool_class.call(arguments.symbolize_keys, project: nil)
    rescue => e
      { error: e.message }
    end
  end
